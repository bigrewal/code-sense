...
def fetch_code_for_def(def_name: str, repo_id: str = "data/xai-sdk-python") -> str:
    """Fetch the code snippet for a given definition name from the codebase."""

    print(f"TOOL INVOKED: Fetching code snippet for definition: {def_name} in repo_id: {repo_id}")

    try:
        neo4j_client = get_neo4j_client()
        
        ## Fetch the start_line, start_column, end_line, end_column for the definition from neo4j.
        ## Write neo4j query where repo_id = "data/xai-sdk-python", is_definition = True, and name = def_name
        query = """
        MATCH (n:Node {repo_id: $repo_id, is_definition: true, name: $def_name})
        ...
        if not code_snippet_coords:
            return f"Definition '{def_name}' not found in the repository."
        ...
        # Stringify the code snippets and return
        return "\n\n".join([f"File: {snippet['file_path']}\n{snippet['code']}" for snippet in code_snippets])
    except Exception as e:
        traceback.print_exc()
        return f"Couldn't fetch code snippet for definition: {def_name} from {repo_id}"
